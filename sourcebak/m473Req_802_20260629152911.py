...
class Servlet:
    ...
    def _query(self):
        ...
        cols = [
            '发货通知单号', '批号', '定尺', '规格', '产品名称1', '牌号',
            '执行标准', '合同号', '收货单位', '日期', '请发日期', '车号',
    ...
    def _insert(self):
        ...
        try:
            sql_name = "SELECT 工资编号+'-'+ISNULL(JEI.姓名,'测试') 姓名" + \
                       " FROM [精益办].dbo.JYB_EmployeeInfo JEI WHERE 工资编号= '" + userid + "'"
    ...
    def _update(self):
        ...
        sql = 'UPDATE ' + self.table + ' SET ' + ','.join(set_clauses) + \
              ' WHERE "发货通知单号" = \'' + key.replace("'", "''") + '\''
        self.db.ExecNonQuery(sql)
    ...
    def _audit(self):
        ...
        sql = 'UPDATE ' + self.table + \
              ' SET "发货状态" = \'已审核\' WHERE "发货通知单号" = \'' + key.replace("'", "''") + '\''
        self.db.ExecNonQuery(sql)
